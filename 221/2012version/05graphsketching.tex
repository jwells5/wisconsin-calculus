%% Time-stamp: Wed Dec 19 14:51:24 2012
...
\begin{figure}[h]
  \centering \input{figures/05tangentAndNormal.tex}
...
\begin{figure}\label{fig:05intermediatevalue}
  \centering \input figures/05intermediatevalue.tex
...
\input figures/05intermediatevalue-fail.tex}
...
\begin{figure}[h]
  \centering\sffamily
  \input{figures/05signs-of-f.pdf_tex}
\end{figure}
...
What about the converse, i.e.\ if you know the sign of $f'$ then what can you
...
\begin{figure}[t]\flushleft\sffamily
  ...
  \hfill
  \parbox[b]{0.4\textwidth}{\input figures/05mvt2.tex }
  ...
  \input{figures/05mvt-proof.pdf_tex}
...
\begin{figure}[h]
  \centering
  \parbox{170pt}{\input figures/05parabola.tex}
  \quad
  \parbox{170pt}{\input figures/05hyperbola.tex}
\end{figure}
...
\begin{figure}[t]
  \centering \input{figures/05upAndDown.tex}
...
\subsection{A function whose tangent turns up and down infinitely often near the origin}
...
\begin{figure}[t]
  \centering \input{figures/05zigzagBetweenParabolas2.tex}
...
There are infinitely many intersection points and their coordinates are
\[
...
a global maximum.
\begin{figure}[h]
  \centering{ \input{figures/05minsAndMaxes.tex}}
...
\subsection{Where to find local maxima and minima}
...
\marginpar{\sffamily\itshape\footnotesize%
  \input figures/05updown-means-max.pdf_tex \\
  \input figures/05downup-means-min.pdf_tex }
\smallskip
...
\marginpar{%
  \input{figures/05neitherMaxNorMin.tex}}
...
\begin{figure}[h]\centering
  \begin{minipage} {110pt}
    \input{figures/05xnomax.tex}
  \end{minipage}
  \hfill
  \begin{minipage}{240pt}
    \input{figures/05xinv_nomin.tex}
  \end{minipage}
...
\input figures/05rationalfunction-slopes.pdf_tex}%
...
\begin{figure}[h]
  \centering \input{figures/05rationalexample.tex}
...
\begin{figure}[h]\centering
  \parbox{115pt}{\input figures/05increasingtangent.tex }
  \parbox{115pt}{\input figures/05decreasingtangent.tex }
  \parbox{115pt}{\input figures/05inflectionpoint.tex }
...
To see if a function is convex the definition asks you to check if the
...
\input figures/05convexgraphwithchord.pdf_tex \\
...
\marginpar{\sffamily\itshape\footnotesize%
  \input figures/05concave-means-max.pdf_tex \\
  \input figures/05convex-means-min.pdf_tex }

\subsection{Example -- that cubic function again}
...
\begin{figure}[h]
  \centering \input{figures/05nonMorsepoints.tex}
...
If $f$ is differentiable then we know what to do: any local maximum is either a
...
\centerline{\includegraphics{figures/05lamppost.pdf}}
...
\problem %max&min of tan x+tan y, with x+y=pi/2
...
\problem \groupproblem %% Josh is in need of coffee.
...
\centerline{\includegraphics{figures/05melita.pdf}}
...
%%\end{trivlist}
%%
%%\problem ({\it van der Waals equation for real gases})
%%
...
x = x(t), \quad y=y(t).
\]%
\marginpar{\input figures/05parametrized-curve.pdf_tex }%
...
\begin{figure}[h]
  \sffamily\small%
  \input figures/05motion-on-a-line.pdf_tex
...
\begin{figure}[t]
  \small%
  \input figures/05motion-on-a-circle.pdf_tex
...
\centerline{\input figures/05motion-on-a-graph.pdf_tex }
...
\begin{figure}[t]\sffamily
  \input figures/05motion-on-a-curve2.pdf_tex
...
\]
\marginpar{\input figures/05small-curved-arc.pdf_tex }%
...
\begin{figure}[ht]
  \centering \input figures/05radius-of-curvature.tex
...
\marginpar{\input figures/05computing-the-radius.pdf_tex }%
\[
...
% \problem Consider the parametrized curve
% \[
...
\[
\lim_{t\searrow a} \frac{f(t)} {g(t)}
\]
that are of the form $\frac{0} {0}$, i.e.~where
\[
\lim_{t\searrow a} f(t) = 0
\text{ and }
\lim_{t\searrow a} g(t) = 0.
\]
It is given by what is called l'Hopital's rule, which says the
following:

\begin{theorem}
  If $f$ and $g$ both are differentiable function, and if
  \[
  \lim_{t\searrow a} \frac{f'(t)} {g'(t)}
  \]
  exists, then $\lim_{t\searrow a} \frac{f(t)} {g(t)} $ also exists.
  Moreover, one has
  \[
  \lim_{t\searrow a} \frac{f(t)} {g(t)} = \lim_{t\searrow a} \frac{f'(t)} {g'(t)}
  \]
\end{theorem}\smallskip

\subsection{The reason why l'Hopital's rule works}
\label{sec:reason-for-lHopital}
The two functions $f$ and $g$ define a parametrized curve $x=g(t)$, $y=f(t)$
that is defined for $t\geq a$.  At $t=0$ you have $x=g(a) = 0$ and $y=g(a) = 0$,
...
\noindent%
\input figures/05mvt-for-lHopital2.pdf_tex%
...
\subsection{Examples of repeated use of l'Hopital's rule}
The limit
\[
...
% LocalWords:  normals parametrized Simplicio
